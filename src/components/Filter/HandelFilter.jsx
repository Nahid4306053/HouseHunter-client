...
export default function HandelFilter({ totaldata }) {
  ...
  const deletefilterkeyword = (key, index) => {
    setSearchParams((old) => {
      key === "brand"
        ? (old.delete("model"), old.delete(key))
        : old.delete(key);
      return old;
    });
        ...
        {keywords.length > 0 && (
          <ul className="filter_item flex gap-5">
            {keywords.map((ele, ind) => {
              return (
                <li
                  className="capitalize bg-base-100 text-sm py-1 rounded-lg px-2 "
                  key={ind}
                >
                  <i
                    onClick={() => deletefilterkeyword(ele.key, ind)}
                    className="fa-solid mr-1 cursor-pointer text-sky-500   fa-circle-xmark"
                  ></i>
                  <strong>{ele.key}: </strong>
                  {
                    ele.value}
                </li>
              );
